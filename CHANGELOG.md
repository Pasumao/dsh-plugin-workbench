...
- **打开右侧详情栏闪一下就消失**。根因：布局补丁把 260px 文件列常驻进拥挤让位链后，
  打开 details 的宽度预算从原生 ~1220px（侧栏280 + 详情300 + 中央640）涨到
  ~1480px——窗口宽度在两者之间的用户点开详情：第一帧渲染 → 拥挤 solve 把 details
  挤成 0 → 面板闪现即消失。修复：**显式打开详情栏时文件列自动让位收起**（
  `panels.details > 0` → explorer 宽 0），宽度预算回到原生水平；关闭详情文件列自动
  恢复。补丁脚本相应新增 `npm-delta3` / `desktop-ci-delta3` 变体（针对已带 0.0.19/20
  occupancy 补丁的 bundle，仅更新 computeCall 一行）。
